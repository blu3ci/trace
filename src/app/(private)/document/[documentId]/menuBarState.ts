...
export function menuBarStateSelector(ctx: EditorStateSnapshot<Editor>) {
  return {
    ...
    // Text Align
    canTextStart: ctx.editor.can().chain().setTextAlign("left").run() ?? false,
    isTextStart: ctx.editor.isActive({ textAlign: "left" }) ?? false,
    canTextCenter:
      ctx.editor.can().chain().setTextAlign("center").run() ?? false,
    isTextCenter: ctx.editor.isActive({ textAlign: "center" }) ?? false,
    canTextEnd: ctx.editor.can().chain().setTextAlign("right").run() ?? false,
    isTextEnd: ctx.editor.isActive({ textAlign: "right" }) ?? false,
